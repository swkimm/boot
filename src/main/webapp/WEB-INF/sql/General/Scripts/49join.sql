...
SELECT e.EmployeeID, AVG(p.Price * od.Quantitiy) 
FROM
	Orders o Join OrderDetails od ON o.OrderID = od.OrderID
			 Join Employees e ON o.EmployeeId = e.EmployeeID
			 Join Products p ON ProductID = p.ProductID 
GROUP BY e.EmployeeID;

-- 카테고리별 평균 상품 가격 조회 (평균 상품 가격이 100.00달러 이상인 것만)
SELECT CategoryId, AVG(Price) average
FROM Products p
GROUP BY CategoryID) T1 JOIN Categories c 
